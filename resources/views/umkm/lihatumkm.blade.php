            ...
            <form method="GET" action="{{ route('umkm.index') }}" class="catalog-form" id="catalogForm">
                ...
                <div class="catalog-filters">
                    ...
                    <div class="filter-group">
                        ...
                        <div class="chip-row">
                            <button type="button" class="chip {{ !request('category') ? 'active' : '' }}" data-field="category" data-value="">
                                Semua
                            </button>

                            @foreach ($categories as $cat)
                                <button type="button" class="chip {{ request('category') === $cat ? 'active' : '' }}" data-field="category" data-value="{{ $cat }}">
                                    {{ $cat }}
                                </button>
                            @endforeach
                        </div>
                    ...
                    <div class="filter-group">
                        ...
                        <div class="chip-row">
                            <button type="button" class="chip {{ !request('type') ? 'active' : '' }}" data-field="type" data-value="">
                                Semua Jenis
                            </button>

                            <button type="button" class="chip {{ request('type') === 'tetap' ? 'active' : '' }}" data-field="type" data-value="tetap">
                                Di Tempat
                            </button>

                            <button type="button" class="chip {{ request('type') === 'keliling' ? 'active' : '' }}" data-field="type" data-value="keliling">
                                Keliling
                            </button>
                        </div>
                    </div>
        ...
        <div class="umkm-grid">
            ...
            @forelse ($umkms as $umkm)
                ...
                <article
                    ...
                    data-opening="{{ $umkm->opening_time }}"
                    data-closing="{{ $umkm->closing_time }}"
            ...
            <nav class="catalog-pagination" aria-label="Paginasi">
                ...
                @if ($last > 1)
                    ...
                    @for ($page = $start; $page <= $end; $page++)
                        <a class="page-btn {{ $current === $page ? 'active' : '' }}" href="{{ $umkms->url($page) }}">{{ $page }}</a>
                    @endfor
                    ...
                    <a class="page-btn {{ $current === $last ? 'active' : '' }}" href="{{ $umkms->url($last) }}">{{ $last }}</a>
